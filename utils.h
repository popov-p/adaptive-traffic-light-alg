...
std::vector<std::shared_ptr<TrafficLight>> initialize_traffic_lights_impl(std::index_sequence<Is...>) {
...
std::vector<std::shared_ptr<TrafficLight>> initialize_traffic_lights() {
    return initialize_traffic_lights_impl<TrafficLightType, StartFromIdx>(std::make_index_sequence<N>{});
}
